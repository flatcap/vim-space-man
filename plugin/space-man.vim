" space-man.vim - Whitespace manager
" Author:       Rich Russon (flatcap) <rich@flatcap.org>
" Website:      https://flatcap.org
" Copyright:    2015 Richard Russon
" License:      GPLv3 <http://fsf.org/>
" Version:      1.0

if (exists ('g:loaded_space_man') || &cp || (v:version < 700))
	finish
endif
let g:loaded_space_man = 1

function! s:translate (func)
	execute 'setlocal operatorfunc=' . a:func
	if (v:count == 0)
		return 'g@'
	else
		return 'g@g@'
	endif
endfunction

function! s:blank_lines_top_map()
	if (get (g:, 'space_man_enable_blank_lines_top', 1))
		nnoremap <Plug>BlankLinesTopA :<C-U>call spaceman#BlankLinesTop ('a')<CR>
		nnoremap <Plug>BlankLinesTopB :<C-U>call spaceman#BlankLinesTop ('b')<CR>
		nnoremap <Plug>BlankLinesTopF :<C-U>call spaceman#BlankLinesTop ('f')<CR>
		nnoremap <Plug>BlankLinesTopW :<C-U>call spaceman#BlankLinesTop ('w')<CR>

		if (get (g:, 'space_man_create_mappings_blank_lines_top', 1))
			nmap <Leader>a<enter>gg <Plug>BlankLinesTopA
			nmap <Leader>b<enter>gg <Plug>BlankLinesTopB
			nmap <Leader>f<enter>gg <Plug>BlankLinesTopF
			nmap <Leader>w<enter>gg <Plug>BlankLinesTopW
		endif
	endif
endfunction

function! s:blank_lines_bottom_map()
	if (get (g:, 'space_man_enable_blank_lines_bottom', 1))
		nnoremap <Plug>BlankLinesBottomA :<C-U>call spaceman#BlankLinesBottom ('a')<CR>
		nnoremap <Plug>BlankLinesBottomB :<C-U>call spaceman#BlankLinesBottom ('b')<CR>
		nnoremap <Plug>BlankLinesBottomF :<C-U>call spaceman#BlankLinesBottom ('f')<CR>
		nnoremap <Plug>BlankLinesBottomW :<C-U>call spaceman#BlankLinesBottom ('w')<CR>

		if (get (g:, 'space_man_create_mappings_blank_lines_bottom', 1))
			nmap <Leader>a<enter>G <Plug>BlankLinesBottomA
			nmap <Leader>b<enter>G <Plug>BlankLinesBottomB
			nmap <Leader>f<enter>G <Plug>BlankLinesBottomF
			nmap <Leader>w<enter>G <Plug>BlankLinesBottomW
		endif
	endif
endfunction

function! s:empty_lines_delete_map()
	if (get (g:, 'space_man_enable_empty_lines_delete', 1))
		nnoremap <expr> <Plug>EmptyLinesDeleteM <SID>translate ('spaceman#EmptyLinesDelete')
		nnoremap        <Plug>EmptyLinesDeleteL :<C-U>call spaceman#EmptyLinesDelete (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>EmptyLinesDeleteV :<C-U>call spaceman#EmptyLinesDelete (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>EmptyLinesDeleteA :<C-U>call spaceman#EmptyLinesDelete ('a')<CR>
		nnoremap        <Plug>EmptyLinesDeleteB :<C-U>call spaceman#EmptyLinesDelete ('b')<CR>
		nnoremap        <Plug>EmptyLinesDeleteF :<C-U>call spaceman#EmptyLinesDelete ('f')<CR>
		nnoremap        <Plug>EmptyLinesDeleteW :<C-U>call spaceman#EmptyLinesDelete ('w')<CR>

		if (get (g:, 'space_man_create_mappings_empty_lines_delete', 1))
			nmap <Leader><enter>dd <Plug>EmptyLinesDeleteL
			nmap <Leader><enter>d  <Plug>EmptyLinesDeleteM
			xmap <Leader><enter>d  <Plug>EmptyLinesDeleteV
			nmap <Leader>a<enter>d <Plug>EmptyLinesDeleteA
			nmap <Leader>b<enter>d <Plug>EmptyLinesDeleteB
			nmap <Leader>f<enter>d <Plug>EmptyLinesDeleteF
			nmap <Leader>w<enter>d <Plug>EmptyLinesDeleteW
		endif
	endif
endfunction

function! s:empty_lines_squeeze_map()
	if (get (g:, 'space_man_enable_empty_lines_squeeze', 1))
		nnoremap <expr> <Plug>EmptyLinesSqueezeM <SID>translate ('spaceman#EmptyLinesSqueeze')
		nnoremap        <Plug>EmptyLinesSqueezeL :<C-U>call spaceman#EmptyLinesSqueeze (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>EmptyLinesSqueezeV :<C-U>call spaceman#EmptyLinesSqueeze (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>EmptyLinesSqueezeA :<C-U>call spaceman#EmptyLinesSqueeze ('a')<CR>
		nnoremap        <Plug>EmptyLinesSqueezeB :<C-U>call spaceman#EmptyLinesSqueeze ('b')<CR>
		nnoremap        <Plug>EmptyLinesSqueezeF :<C-U>call spaceman#EmptyLinesSqueeze ('f')<CR>
		nnoremap        <Plug>EmptyLinesSqueezeW :<C-U>call spaceman#EmptyLinesSqueeze ('w')<CR>

		if (get (g:, 'space_man_create_mappings_empty_lines_squeeze', 1))
			nmap <Leader><enter>zz <Plug>EmptyLinesSqueezeL
			nmap <Leader><enter>z  <Plug>EmptyLinesSqueezeM
			xmap <Leader><enter>z  <Plug>EmptyLinesSqueezeV
			nmap <Leader>a<enter>z <Plug>EmptyLinesSqueezeA
			nmap <Leader>b<enter>z <Plug>EmptyLinesSqueezeB
			nmap <Leader>f<enter>z <Plug>EmptyLinesSqueezeF
			nmap <Leader>w<enter>z <Plug>EmptyLinesSqueezeW
		endif
	endif
endfunction

function! s:space_error_map()
	if (get (g:, 'space_man_enable_space_error', 1))
		nnoremap <expr> <Plug>SpaceErrorM <SID>translate ('spaceman#SpaceError')
		nnoremap        <Plug>SpaceErrorL :<C-U>call spaceman#SpaceError (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>SpaceErrorV :<C-U>call spaceman#SpaceError (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>SpaceErrorA :<C-U>call spaceman#SpaceError ('a')<CR>
		nnoremap        <Plug>SpaceErrorB :<C-U>call spaceman#SpaceError ('b')<CR>
		nnoremap        <Plug>SpaceErrorF :<C-U>call spaceman#SpaceError ('f')<CR>
		nnoremap        <Plug>SpaceErrorW :<C-U>call spaceman#SpaceError ('w')<CR>

		if (get (g:, 'space_man_create_mappings_space_error', 1))
			nmap <Leader><space>ee <Plug>SpaceErrorL
			nmap <Leader><space>e  <Plug>SpaceErrorM
			xmap <Leader><space>e  <Plug>SpaceErrorV
			nmap <Leader>a<space>e <Plug>SpaceErrorA
			nmap <Leader>b<space>e <Plug>SpaceErrorB
			nmap <Leader>f<space>e <Plug>SpaceErrorF
			nmap <Leader>w<space>e <Plug>SpaceErrorW
		endif
	endif
endfunction

function! s:space_leading_map()
	if (get (g:, 'space_man_enable_space_leading', 1))
		nnoremap <expr> <Plug>SpaceLeadingM <SID>translate ('spaceman#SpaceLeading')
		nnoremap        <Plug>SpaceLeadingL :<C-U>call spaceman#SpaceLeading (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>SpaceLeadingV :<C-U>call spaceman#SpaceLeading (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>SpaceLeadingA :<C-U>call spaceman#SpaceLeading ('a')<CR>
		nnoremap        <Plug>SpaceLeadingB :<C-U>call spaceman#SpaceLeading ('b')<CR>
		nnoremap        <Plug>SpaceLeadingF :<C-U>call spaceman#SpaceLeading ('f')<CR>
		nnoremap        <Plug>SpaceLeadingW :<C-U>call spaceman#SpaceLeading ('w')<CR>

		if (get (g:, 'space_man_create_mappings_space_leading', 1))
			nmap <Leader><space>ll <Plug>SpaceLeadingL
			nmap <Leader><space>l  <Plug>SpaceLeadingM
			xmap <Leader><space>l  <Plug>SpaceLeadingV
			nmap <Leader>a<space>l <Plug>SpaceLeadingA
			nmap <Leader>b<space>l <Plug>SpaceLeadingB
			nmap <Leader>f<space>l <Plug>SpaceLeadingF
			nmap <Leader>w<space>l <Plug>SpaceLeadingW
		endif
	endif
endfunction

function! s:space_trailing_map()
	if (get (g:, 'space_man_enable_space_trailing', 1))
		nnoremap <expr> <Plug>SpaceTrailingM <SID>translate ('spaceman#SpaceTrailing')
		nnoremap        <Plug>SpaceTrailingL :<C-U>call spaceman#SpaceTrailing (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>SpaceTrailingV :<C-U>call spaceman#SpaceTrailing (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>SpaceTrailingA :<C-U>call spaceman#SpaceTrailing ('a')<CR>
		nnoremap        <Plug>SpaceTrailingB :<C-U>call spaceman#SpaceTrailing ('b')<CR>
		nnoremap        <Plug>SpaceTrailingF :<C-U>call spaceman#SpaceTrailing ('f')<CR>
		nnoremap        <Plug>SpaceTrailingW :<C-U>call spaceman#SpaceTrailing ('w')<CR>

		if (get (g:, 'space_man_create_mappings_space_trailing', 1))
			nmap <Leader><space>tt <Plug>SpaceTrailingL
			nmap <Leader><space>t  <Plug>SpaceTrailingM
			xmap <Leader><space>t  <Plug>SpaceTrailingV
			nmap <Leader>a<space>t <Plug>SpaceTrailingA
			nmap <Leader>b<space>t <Plug>SpaceTrailingB
			nmap <Leader>f<space>t <Plug>SpaceTrailingF
			nmap <Leader>w<space>t <Plug>SpaceTrailingW
		endif
	endif
endfunction

function! s:spaces_tabs_map()
	if (get (g:, 'space_man_enable_spaces_tabs', 1))
		nnoremap <expr> <Plug>SpacesTabsM <SID>translate ('spaceman#SpacesTabs')
		nnoremap        <Plug>SpacesTabsL :<C-U>call spaceman#SpacesTabs (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>SpacesTabsV :<C-U>call spaceman#SpacesTabs (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>SpacesTabsA :<C-U>call spaceman#SpacesTabs ('a')<CR>
		nnoremap        <Plug>SpacesTabsB :<C-U>call spaceman#SpacesTabs ('b')<CR>
		nnoremap        <Plug>SpacesTabsF :<C-U>call spaceman#SpacesTabs ('f')<CR>
		nnoremap        <Plug>SpacesTabsW :<C-U>call spaceman#SpacesTabs ('w')<CR>

		if (get (g:, 'space_man_create_mappings_spaces_tabs', 1))
			nmap <Leader><space><tab><tab> <Plug>SpacesTabsL
			nmap <Leader><space><tab>      <Plug>SpacesTabsM
			xmap <Leader><space><tab>      <Plug>SpacesTabsV
			nmap <Leader>a<space><tab>     <Plug>SpacesTabsA
			nmap <Leader>b<space><tab>     <Plug>SpacesTabsB
			nmap <Leader>f<space><tab>     <Plug>SpacesTabsF
			nmap <Leader>w<space><tab>     <Plug>SpacesTabsW
		endif
	endif
endfunction

function! s:tabs_spaces_map()
	if (get (g:, 'space_man_enable_tabs_spaces', 1))
		nnoremap <expr> <Plug>TabsSpacesM <SID>translate ('spaceman#TabsSpaces')
		nnoremap        <Plug>TabsSpacesL :<C-U>call spaceman#TabsSpaces (line ('.'),  line ('.'))<CR>
		xnoremap        <Plug>TabsSpacesV :<C-U>call spaceman#TabsSpaces (line ("'<"), line ("'>"))<CR>
		nnoremap        <Plug>TabsSpacesA :<C-U>call spaceman#TabsSpaces ('a')<CR>
		nnoremap        <Plug>TabsSpacesB :<C-U>call spaceman#TabsSpaces ('b')<CR>
		nnoremap        <Plug>TabsSpacesF :<C-U>call spaceman#TabsSpaces ('f')<CR>
		nnoremap        <Plug>TabsSpacesW :<C-U>call spaceman#TabsSpaces ('w')<CR>

		if (get (g:, 'space_man_create_mappings_tabs_spaces', 1))
			nmap <Leader><tab><space><space> <Plug>TabsSpacesL
			nmap <Leader><tab><space>        <Plug>TabsSpacesM
			xmap <Leader><tab><space>        <Plug>TabsSpacesV
			nmap <Leader>a<tab><space>       <Plug>TabsSpacesA
			nmap <Leader>b<tab><space>       <Plug>TabsSpacesB
			nmap <Leader>f<tab><space>       <Plug>TabsSpacesF
			nmap <Leader>w<tab><space>       <Plug>TabsSpacesW
		endif
	endif
endfunction

function! s:space_surround_map()
	if (get (g:, 'space_man_enable_space_surround', 1))
		nnoremap <expr> <Plug>SpaceSurroundM <SID>translate ('spaceman#SpaceSurroundMotion')
		nnoremap        <Plug>SpaceSurroundL :<C-U>call spaceman#SpaceSurroundLine (line ('.'), line ('.'), v:count1)<CR>
		xnoremap        <Plug>SpaceSurroundV :<C-U>call spaceman#SpaceSurroundVisual()<CR>

		if (get (g:, 'space_man_create_mappings_space_surround', 1))
			nmap <Leader><space>ss <Plug>SpaceSurroundL
			nmap <Leader><space>s  <Plug>SpaceSurroundM
			xmap <Leader><space>s  <Plug>SpaceSurroundV
		endif
	endif
endfunction


call s:blank_lines_top_map()
call s:blank_lines_bottom_map()
call s:empty_lines_delete_map()
call s:empty_lines_squeeze_map()
call s:space_error_map()
call s:space_leading_map()
call s:space_trailing_map()
call s:spaces_tabs_map()
call s:tabs_spaces_map()
call s:space_surround_map()

